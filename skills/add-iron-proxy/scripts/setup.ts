...
function addAllowedHost(host: string, projectRoot: string): void {
  ...
  const hosts = [
    ...new Set([...readAllowedHosts(projectRoot), validateAllowedHost(host)]),
  ].sort();
...
export function renderConfig(
  ...
  const domains = [
    ...new Set([
      validateAllowedHost(modelHost),
      ...allowedHosts.map(validateAllowedHost),
    ]),
  ].sort();
  return `dns:
  enabled: false
...
async function configuredPort(projectRoot: string): Promise<number> {
  const existing = Number(
    process.env.NANOCLAW_IRON_PROXY_PORT ||
      readProjectEnv(projectRoot).NANOCLAW_IRON_PROXY_PORT,
  );
  if (Number.isInteger(existing) && existing >= 1024 && existing <= 65535)
    return existing;
  let port = await availablePort();
  while (port === DIRECT_HTTP_PORT || port === DIRECT_HTTPS_PORT)
...
function ensureCA(projectRoot: string): void {
  ...
  if (hasCert !== hasKey)
    throw new Error(`Iron Proxy CA is incomplete under ${paths.stateDir}`);
  if (!hasCert) {
...
function ensureSecretsFile(projectRoot: string): void {
  ...
  if (!fs.existsSync(file))
    fs.writeFileSync(file, "IRON_UPSTREAM_CLAUDE=not-configured\n", {
      mode: 0o600,
    });
...
function writeConfig(
  ...
  fs.writeFileSync(
    file,
    renderConfig(port, modelHost, readAllowedHosts(projectRoot)),
    { mode: 0o600 },
  );
  fs.chmodSync(file, 0o600);
}
...
function removeOwnedContainer(projectRoot: string): void {
  ...
  try {
    owner = docker(
      [
        "inspect",
        "--format",
        '{{ index .Config.Labels "nanoclaw-install" }}',
        name,
      ],
      { stdout: true },
    );
  ...
  if (owner !== expected)
    throw new Error(
      `Refusing to replace container ${name}: ownership label is '${owner || "(missing)"}'`,
    );
...
async function waitRunning(name: string): Promise<void> {
  ...
  throw new Error(
    `Iron Proxy did not stay running (${name})${logs ? `: ${logs}` : ""}`,
  );
}
...
export async function startProxy(projectRoot = process.cwd()): Promise<void> {
  ...
  if (!Number.isInteger(port))
    throw new Error("Iron Proxy port is not configured");
  const paths = statePaths(projectRoot);
  const name = containerName(projectRoot);
  docker(["pull", IMAGE]);
...
export async function configureCredential(
  ...
  if (
    ...
    ].includes(credential.authEnv)
  ) {
    throw new Error(`Unsupported Iron Proxy auth env: ${credential.authEnv}`);
  }
  if (!credential.secret || /[\r\n]/.test(credential.secret))
    throw new Error("Credential must be one non-empty line");
  const paths = statePaths(projectRoot);
  fs.writeFileSync(
    paths.secrets,
    `IRON_UPSTREAM_CLAUDE=${credential.secret}\n`,
    { mode: 0o600 },
  );
  ...
  upsertEnvVar(
    ...
    validateAllowedHost(credential.modelHost),
    projectRoot,
  );
  if (credential.baseUrl)
    upsertEnvVar("ANTHROPIC_BASE_URL", credential.baseUrl, projectRoot);
  writeConfig(
    projectRoot,
    Number(readProjectEnv(projectRoot).NANOCLAW_IRON_PROXY_PORT),
    credential.modelHost,
  );
...
export async function run(
  ...
  if (allowIndex >= 0) {
    if (!args[allowIndex + 1])
      throw new Error("--allow-host requires a hostname or *.domain");
    addAllowedHost(args[allowIndex + 1], projectRoot);
  }
  const port = await configuredPort(projectRoot);
  const name = containerName(projectRoot);
  const modelHost =
    readProjectEnv(projectRoot).NANOCLAW_IRON_PROXY_MODEL_HOST ||
    DEFAULT_MODEL_HOST;
