...
function existingCredential(): boolean {
  const file = path.join(process.cwd(), 'data', 'gateways', 'iron-proxy', 'secrets.env');
  if (!fs.existsSync(file)) return false;
  const value = fs.readFileSync(file, 'utf8').match(/^IRON_UPSTREAM_CLAUDE=(.*)$/m)?.[1];
  return !!value && value !== 'not-configured';
}

function customEndpoint(): { secret: string; authEnv: string; modelHost: string; baseUrl: string } | null {
  ...
  return { secret, authEnv: 'ANTHROPIC_AUTH_TOKEN', modelHost: url.hostname, baseUrl: url.toString().replace(/\/$/, '') };
}
