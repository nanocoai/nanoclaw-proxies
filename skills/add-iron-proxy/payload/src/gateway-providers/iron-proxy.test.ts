...
import { IronProxyProvider, ironProxyContribution, readIronProxySettings } from './iron-proxy.js';
...
describe('IronProxyProvider', () => {
  ...
  it('refuses invalid setup state', () => {
    expect(() => readIronProxySettings({ NANOCLAW_IRON_PROXY_PORT: '80' }, '/tmp/project')).toThrow(
      /valid unprivileged port/,
    );
    expect(() =>
      readIronProxySettings(
        { NANOCLAW_IRON_PROXY_PORT: '18080', NANOCLAW_IRON_PROXY_AUTH_ENV: 'ARBITRARY_SECRET' },
        '/tmp/project',
      ),
    ).toThrow(/unsupported/);
  });
