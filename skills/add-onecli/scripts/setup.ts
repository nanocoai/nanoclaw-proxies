...
function installOnecliCliDirect(): { stdout: string; ok: boolean } {
  ...
  try {
    ...
    execSync(
      `tar -xzf ${JSON.stringify(archivePath)} -C ${JSON.stringify(tmpDir)}`,
      {
        stdio: ["ignore", "pipe", "pipe"],
      },
    );
 ...
 */
function gatewayV1Hint(
  result: "ok" | "incompatible" | "unreachable",
): string | null {
...
export async function run(args: string[]): Promise<void> {
  ...
  if (requestedRemote && !args.includes("--remote-url"))
    args = [...args, "--remote-url", requestedRemote];
  const reuse =
    args.includes("--reuse") ||
    (!requestedRemote && !!onecliVersion() && !!getOnecliApiHost());
  ...
  if (remoteUrl) {
    ...
    const v1Hint = healthy
      ? gatewayV1Hint(await verifyGatewayV1(remoteUrl))
      : null;
    emitStatus("ONECLI", {
      INSTALLED: true,
      REMOTE: true,
